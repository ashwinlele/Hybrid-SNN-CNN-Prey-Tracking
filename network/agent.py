...
class PedraAgent():
    ...
    def take_action(self, action, iteration, num_actions, SimMode):
        ...
        if SimMode == 'Multirotor':
            ...
            ## circular
            if iteration < 300:
                vy =    -0.002 + r_infer *  random.uniform(-1, 1) #0.003 + r_infer *  random.uniform(-1, 1) #0.0001 + r_infer *  random.uniform(-1, 1) #np.cos(alpha + psi)
                vx = 0.002 + r_infer *  random.uniform(-1, 1) #min(iteration*0.00001,0.003) + r_infer *  random.uniform(-1, 1) #0.0003 + r_infer *  random.uniform(-1, 1) #np.sin(alpha + psi)
            
            #if iteration < 300:
            #    vy = -0.004*(math.cos(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #0.003 + r_infer *  random.uniform(-1, 1) #0.0001 + r_infer *  random.uniform(-1, 1) #np.cos(alpha + psi)
            #    vx = 0.004*abs(math.sin(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #min(iteration*0.00001,0.003) + r_infer *  random.uniform(-1, 1) #0.0003 + r_infer *  random.uniform(-1, 1) #np.sin(alpha + psi)
            '''
            if iteration < 300:
                vy = -0.002 + r_infer *  random.uniform(-1, 1) #0.003 + r_infer *  random.uniform(-1, 1) #0.0001 + r_infer *  random.uniform(-1, 1) #np.cos(alpha + psi)
                vx = 0.002 + r_infer *  random.uniform(-1, 1) #min(iteration*0.00001,0.003) + r_infer *  random.uniform(-1, 1) #0.0003 + r_infer *  random.uniform(-1, 1) #np.sin(alpha + psi)
            ...
            if iteration < 300:
                vy = 0.004*abs(math.cos(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #0.003 + r_infer *  random.uniform(-1, 1) #0.0001 + r_infer *  random.uniform(-1, 1) #np.cos(alpha + psi)
                vx = 0.004*abs(math.sin(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #min(iteration*0.00001,0.003) + r_infer *  random.uniform(-1, 1) #0.0003 + r_infer *  random.uniform(-1, 1) #np.sin(alpha + psi)
            
            ## sin
            if iteration < 300:
                vy = 0.003* abs(math.cos(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #0.003 + r_infer *  random.uniform(-1, 1) #0.0001 + r_infer *  random.uniform(-1, 1) #np.cos(alpha + psi)
                vx = 0.003 + r_infer *  random.uniform(-1, 1) #min(iteration*0.00001,0.003) + r_infer *  random.uniform(-1, 1) #0.0003 + r_infer *  random.uniform(-1, 1) #np.sin(alpha + psi)
            
            ## circular
            if iteration < 300:
                vy = -0.003*(math.cos(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #0.003 + r_infer *  random.uniform(-1, 1) #0.0001 + r_infer *  random.uniform(-1, 1) #np.cos(alpha + psi)
                vx = -0.003*(math.sin(iteration*3.14*2/200)) + r_infer *  random.uniform(-1, 1) #min(iteration*0.00001,0.003) + r_infer *  random.uniform(-1, 1) #0.0003 + r_infer *  random.uniform(-1, 1) #np.sin(alpha + psi)
            ...
            '''
            vz = -0.00305   #+ r_infer/12 *  random.uniform(-0.5, 0.5) #np.sin(theta)
            x = pos.x_val
            y = pos.y_val
            z = pos.z_val
            ...
            self.client.moveByVelocityAsync(vx=5*vx, vy=5*vy, vz=vz, duration=2, #0.0003,
    ...
    def take_action2(self, action, num_actions,vx,vy,vz, yaw_speed,offset_x,offset_y,SimMode):
        ...
        if SimMode == 'ComputerVision':
            ...
            #self.client.moveByAngleRatesZAsync(0, 1,1, 0, duration=0.00003, vehicle_name=self.vehicle_name)
            self.client.hoverAsync().join()
